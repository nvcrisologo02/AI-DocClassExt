...
public class NormalizarActivityTests
{
    ...
    {
        _sut = new NormalizarActivity(new Mock<ILogger<NormalizarActivity>>().Object);
    }
    ...
    [Fact]
    public void Run_ArrayVacio_CRC32EsCero()
    {
    ...
    [Fact]
    public void Run_BytesConocidos_TamañoBytesEsCorrecto()
    {
    ...
    [Fact]
    public void Run_NombreConEspaciosYMayusculas_NombreNormalizadoEnMinusculasSinEspacios()
    {
        var bytes = new byte[] { 1 };

        var result = _sut.Run(BuildEntrada(bytes, "  Mi Documento.PDF  "));

        result["NombreNormalizado"].Should().Be("mi documento.pdf");
    }

    [Fact]
    public void Run_NombreYaMinusculas_NombreNormalizadoSinCambios()
    {
        var bytes = new byte[] { 1 };

        var result = _sut.Run(BuildEntrada(bytes, "documento.pdf"));

        result["NombreNormalizado"].Should().Be("documento.pdf");
    }

    [Fact]
    public void Run_Resultado_ContieneFechaNormalizacionReciente()
    {
        var bytes = new byte[] { 1 };
    ...
    [Fact]
    public void Run_Resultado_ContieneTodosLasClavesEsperadas()
    {
    ...
    [Fact]
    public void Run_MismoContenido_RetornaMismosHashesDeterministas()
    {
        var bytes = new byte[] { 10, 20, 30, 40 };
        var entrada = BuildEntrada(bytes);

        var result1 = _sut.Run(entrada);
        var result2 = _sut.Run(entrada);
    ...
    [Fact]
    public void Run_ContenidoDiferente_RetornaHashesDiferentes()
    {
        var bytes1 = new byte[] { 1, 2, 3 };
        var bytes2 = new byte[] { 4, 5, 6 };

        var result1 = _sut.Run(BuildEntrada(bytes1));
        var result2 = _sut.Run(BuildEntrada(bytes2));
