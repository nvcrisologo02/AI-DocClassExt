/*  Variante del harness SIN CREATE/DROP DATABASE (entornos sin permiso CREATE DATABASE).
    Todo ocurre sobre una tabla temporal local #Tipologias de la sesion actual; NUNCA
    lee ni escribe dbo.Tipologias real. Ejecutar el fichero ENTERO (F5) en UNA sola
    ventana/conexion de SSMS: las tablas temporales locales persisten entre lotes GO
    dentro de la misma sesion. No requiere SQLCMD Mode.

    Conectate a la BBDD DocumentIA del entorno (o a cualquier BBDD con nivel de
    compatibilidad >=130 y motor >=2017, por OPENJSON/STRING_AGG). Es seguro conectarse
    a la DocumentIA real: este script solo usa #Tipologias, no toca los datos de
    produccion. Exito = en Messages: OK: descubrimiento / aplicacion / idempotencia /
    rollback, sin ningun Msg 5000x. */
SET NOCOUNT ON;
GO
-- Reset idempotente de la sesion (por si se reejecuta sin reconectar)
IF OBJECT_ID('tempdb..#Tipologias')    IS NOT NULL DROP TABLE #Tipologias;
IF OBJECT_ID('tempdb..#Descubrimiento') IS NOT NULL DROP TABLE #Descubrimiento;
...
INSERT INTO #Tipologias (Codigo, Version, Estado, Activa, ConfiguracionJson) VALUES
...
-- CROSS APPLY ya invocó OPENJSON sobre todas las filas de #Tipologias, incluida
...
IF NOT EXISTS (SELECT 1 FROM #Descubrimiento d JOIN #Tipologias t ON t.Id=d.Id
...
IF EXISTS (SELECT 1 FROM #Descubrimiento d JOIN #Tipologias t ON t.Id=d.Id
...
SELECT Id, ConfiguracionJson INTO #orig FROM #Tipologias;
GO
...
INTO #obj
FROM (SELECT DISTINCT Id FROM Qualified) q
JOIN #Tipologias t ON t.Id = q.Id AND t.Estado = 1 AND t.Activa = 1;
...
IF NOT EXISTS (SELECT 1 FROM #Tipologias
...
IF JSON_QUERY((SELECT ConfiguracionJson FROM #Tipologias WHERE Codigo='ibi-flat-refcat'),'$.assetResolver.mapeoIdufir') IS NOT NULL
    BEGIN PRINT 'FAIL: ibi-flat-refcat no debe tener mapeoIdufir'; THROW 50003,'ibi mapeoIdufir',1; END

-- c) decoy sin assetResolver
IF JSON_QUERY((SELECT ConfiguracionJson FROM #Tipologias WHERE Codigo='valor-decoy'),'$.assetResolver') IS NOT NULL
...
SELECT Id, ConfiguracionJson INTO #snap FROM #Tipologias;
GO
...
IF EXISTS (SELECT 1 FROM #Tipologias t JOIN #snap s ON s.Id=t.Id
...
DECLARE @fail INT = 0;
IF EXISTS (SELECT 1 FROM #Tipologias t JOIN #orig o ON o.Id=t.Id
