/*  Activa AssetResolver en tipologias Publicadas/Activas cuyo esquema declare
    ...
    VARIANTE SSMS (ventana normal, sin SQLCMD Mode): en vez de sqlcmd -v, edita la
    linea "DECLARE @WhatIf BIT = 1;" de mas abajo. Deja 1 para DRY-RUN (muestra las
    tipologias afectadas y hace ROLLBACK); ponla a 0 para APLICAR (COMMIT). Ejecuta el
    fichero entero (F5) conectado a la BBDD DocumentIA del entorno.
