...
El schema se gestiona mediante migraciones de EF Core Code-First. Caracteristicas principales del modelo vigente:

- Tablas centrales: `Documentos`, `Tipologias`, `ResultadosProcesamiento`, `Auditoria`.
- Historial de ejecuciones: `DocumentoEjecuciones`, `PluginEjecuciones`, `ValidacionResultados`, con seguimiento de orquestación (`ActivityTimeline`, duraciones por actividad).
- Configuración dinámica: `TipologiaConfigAudit`, `ModeloConfigs`, `PluginTipologiaConfigs`.
- Catálogos de clasificación jerárquica: `CatalogoTdn1` (con `TDN2_Prompt`), `CatalogoTdn2`.
- Campos de integridad y trazabilidad: `MD5`, `NormalizacionMarkdownCompressed`, `InstanceId`, `OperationId` (W3C TraceId), `NivelClasificacion`, `ClassificationOnly`.
- Resolución de activo: `AssetResolverResultJson`, `DuracionAssetResolverMs`.
- Retención: `FechaExpiracionBlob` y políticas asociadas.

El campo `PromptGPT` está deprecado; la configuración de prompts se gestiona mediante `ConfiguracionJson.promptConfig`.
...
## 8. Vistas Recomendadas (opcionales)
