...
**Importante: PromptGPT Field Deprecated**

In v1.4, the direct `.promptGPT` field is **deprecated but supported** for backward compatibility. All prompt templates are managed via `configuracionJson.promptConfig`:

| Property | Estado |
|----------|--------|
| `.promptGPT` | Deprecated (read-only, soportado por compatibilidad) |
| `configuracionJson.promptConfig.systemPrompt` | Primary |
| `configuracionJson.promptConfig.userPromptTemplate` | Primary |

**Migration Impact:** Existing Tipologías continue to work. Update `configuracionJson` to adopt the current standard.
...
- **Cannot update:** `id`, `estado`, `creadoPor`, `fechaCreacion`
...
**Nota — Formato de respuesta:** El campo directo `.promptGPT` se **omite** de la respuesta (se mantiene internamente via extension methods).
...
|---------|--------|-----------------|
| `.promptGPT` field (direct read) | Deprecated | Use `configuracionJson.promptConfig.systemPrompt` |
| Legacy DTO (with `.promptGPT`) | Supported | Not recommended for new code |
| Direct field access | Deprecated | Use extension methods: `tipologia.GetSystemPrompt()` |
...
(See 12_MIGRACION_PROMPTGPT_V1_4.md for full guide)
...
**A:** There's a separate `POST /management/tipologias/import` endpoint for ZIP imports. See 04_MANUAL_EXPLOTACION.md for details.
...
- For merge conflict handling, see 12_MIGRACION_PROMPTGPT_V1_4.md
...
- **Questions:** See 12_MIGRACION_PROMPTGPT_V1_4.md for detailed migration guide
