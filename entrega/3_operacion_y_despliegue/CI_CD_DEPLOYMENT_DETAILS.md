...
**Status:** Verificado contra azure-pipelines*.yml
...
### Current State
- **Nota:** No hay CI/CD automático — todos los pipelines son de ejecución manual
- Los despliegues se parametrizan por `targetEnvironment` (`dev`/`pre`/`prod`); cada entorno tiene su propio Resource Group, Service Connection y Key Vault
- Service Connection por entorno: "AI DocClassExt DEV" / "AI DocClassExt PRE" / "AI DocClassExt PRO"
...
**Current Status:** DISABLED (condition: false)
...
│  └─→ Option 2: Stop → Restore DB → Revert code  DOWNTIME
│
...
if ($errors -lt 3) {
  ...
  Write-Output "Elevated error rate ($errors exceptions)"
}
...
if ($latency -lt 2000) {  # 2000ms = 2s
  ...
  Write-Output "Elevated latency ($latency ms)"
}
...
6. Run azure-pipelines.yml → All stages pass
7. Monitor 24+ hours
...
| Role | Contact | When |
|------|---------|------|
| **Deployment Lead** | Responsable del despliegue en curso | Started or on-going deployment |
| **Tech Lead** | Responsable técnico | Deployment fails, needs decision |
| **Responsable de operaciones / dirección técnica** | Escalado de decisión | P1 issue, rollback decision required |
| **Administración de base de datos** | Responsable de BD | Database issues, restore needed |

**Canal de incidencias:** #incidentes-produccion
...
**Related Docs:**
- [INFRAESTRUCTURA_REAL_DESPLEGADA.md](INFRAESTRUCTURA_REAL_DESPLEGADA.md) — Full infrastructure topology
