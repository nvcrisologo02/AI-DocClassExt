...
| # | Tarea | Detalle | OK |
|---|-------|---------|----|
| S.1 | Activar App Service Authentication (EasyAuth) en el Admin | App registration single-tenant + grupo de seguridad con los usuarios autorizados y "Assignment required = Yes". Solicitud al equipo de identidad y guia de configuracion del App Service documentadas internamente (`docs/guias/SOLICITUD_APP_REGISTRATION_ADMIN.md` y `docs/guias/GUIA_EASYAUTH_ADMIN.md` en el repositorio). **DEV ya dispone de app registration (`DocumentIA Admin - DEV`) y grupo (`GSEC-DocumentIA-Admin-DEV`) verificados; datos y comandos en la seccion "Datos y comandos por entorno" de la guia. Falta activar Authentication en el App Service. PRE y PROD pendientes de solicitud.** | ☐ |
