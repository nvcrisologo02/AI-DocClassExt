...
flowchart TD
    ...
    F -->|"NO_CLASIFICADO"| G["⚠️ FIN: NO_CLASIFICADO\nDocumento no identificable"]
...
flowchart TD
    ...
    C -->|"NO_CLASIFICADO"| D["⚠️ FIN: NO_CLASIFICADO"]
...
Se activa mediante el parámetro `instrucciones.classification.classificationOnly` (o el script dedicado) con la instrucción de no ejecutar extracción:
...
flowchart LR
    ...
    F -->|"Desconocido / baja confianza"| H["⚠️ NO_CLASIFICADO"]
```
...
| GPT devuelve "Desconocido" o confianza < 0.3 (RESTO) | ⚠️ `NO_CLASIFICADO` (fin controlado) |
...
flowchart TD
    ...
    G -->|"'Desconocido'\nconfianza < 0.3\nEjemplo: RESTO original"| I["⚠️ NO_CLASIFICADO\nEstado controlado\nno es error técnico"]
...
|---|---|---|---|---|---|---|
| `nota-simple` (v1.4, default) | `nota.simple.1_4` | NOTS/NOTS01 | Azure CU | ✅ gpt4o-mini | No | ✅ |
| `nota-simple` (v1.0, legacy) | `nota.simple` | NOTS/NOTS01 | Ninguna | ❌ | No | ✅ |
| `tasacion` | `tasacion` | — | Ninguna | ❌ | **Sí** | ✅ |
| `resumen-documental` | `resumen.documental` | — | Mock | ✅ vision | **Sí** | ✅ |
| `IBI` (v1.1, DB-backed) | `IBI_1.1` | — | Azure OpenAI | ❌ | **Sí** | ✅ |
| `general-prompting` | `general-prompting` | — | — | ✅ | **Sí** | ❌ |
...
|---|---|---|---|---|
| `ESCR-01` | `escr.titularidad.otro` | Escrituras | Cancelación hipoteca, carta de pago | ✅ |
| `ESCR-06` | `escr.titularidad.otro` | Escrituras | Dación en pago, adjudicación | ✅ |
| `ESCR-10` | `escr.compraventa` | Escrituras | Compraventa inmobiliaria | ✅ |
| `ESCR-26` | `escr.titularidad.otro` | Escrituras | Titularidad residual (dominio, propiedad) | ✅ |
| `ESCR-29` | `escr.titularidad.otro` | Escrituras | Titularidad acreditada contractual/notarial | ✅ |
| `DOCN-06` | `docn.complementario` | Documentos notariales | Acta/escritura complementaria o subsanación | ✅ |
| `SERE-24` | `sere.ejecucion` | Sentencias/Resoluciones | Ejecución hipotecaria judicial | ✅ |
| `SERE-25` | `sere.cancelacion` | Sentencias/Resoluciones | Mandamiento cancelación cargas | ✅ |
| `SERE-26` | `sere.adjudicacion` | Sentencias/Resoluciones | Testimonio judicial de adjudicación | ✅ |
...
| Estado | Rango de confianza | Color semáforo | Descripción |
|---|---|---|---|
...
| 9 | DI <0.85 | GPT ✅ | Disabled | — | ✅ `OK` |
| 10 | DI <0.85 | GPT ✅ | Provider | ≥50% campos | ✅ `OK` |
| 11 | DI <0.85 | GPT ✅ | Provider | <50% campos | ✅ `OK` (fallback GPT extracción) |
| 12 | DI <0.85 | GPT ✅ | Provider | Error | ❌ `ERROR` |
| 13 | DI <0.85 | GPT ❌ | Disabled | — | ✅ `OK` (mantiene resultado DI) |
| 14 | DI <0.85 | GPT ❌ | Provider | ≥50% campos | ✅ `OK` |
| 15 | DI <0.85 | GPT ❌ | Provider | <50% campos | ✅ `OK` (fallback GPT extracción) |
| 16 | DI error | GPT ✅ | Provider | ≥50% campos | ✅ `OK` |
| 17 | DI error | GPT ✅ | Provider | <50% campos | ✅ `OK` (fallback GPT) |
| 18 | DI error | GPT ✅ | Provider | Error | ❌ `ERROR` |
| 19 | Mock | N/A | Provider | ≥50% campos | ✅ `OK` |
| 20 | DI error | GPT ❌ | — | — | ❌ `ERROR` |
| 21 | DI = RESTO | GPT ✅ | Provider | ≥50% campos | ✅ `OK` |
| 22 | DI = RESTO | GPT ❌ (Desconocido) | — | — | ⚠️ `NO_CLASIFICADO` |
...
flowchart TD
    ...
    F -->|"GPT → Desconocido\n(RESTO original)"| NC["⚠️ NO_CLASIFICADO\n(#22)"]
...
DI → "RESTO" (confianza: 0.40)
    → Fallback GPT obligatorio
        ...
        → GPT → "Desconocido" (confianza: 0.15) → ⚠️ NO_CLASIFICADO (controlado)
```
