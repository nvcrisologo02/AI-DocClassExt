...
Bienvenido 👋. En los próximos **15 minutos** tendrás el proyecto corriendo localmente con tu primer clasificador ejecutado.

---

## 0. Prerequisites *(2 minutos)*
...
Si todos responden con versiones ✅, adelante.

---

## 1. Clone & Setup *(5 minutos)*
...
## 2. Estructura de Carpetas Clave *(2 minutos)*
...
## 3. Arrancar Localmente *(3 minutos)*
...
## 4. Tu Primer Clasificador *(2 minutos)*
...
## 5. Debugging & Tips de Productividad *(1-2 minutos)*

### Breakpoints en VS Code

1. Abre `src/backend/DocumentIA.Functions/IngestDocument.cs`
2. Haz clic en el margen izquierdo (número de línea) para marcar un breakpoint (rojo •)
...
sqlcmd -S localhost,1433 -U sa -P tu_password -Q "SELECT TOP 10 * FROM [DocumentIA].[dbo].[Executions]"
```
...
Ahora que tienes el sistema corriendo, profundiza:

1. **Entiende el vocabulario:**
   - Lee [docs/GLOSSARIO_TERMINOS.md](../GLOSSARIO_TERMINOS.md) (5 min)
   - Aprende qué es Orquestación, Actividad, Instancia, etc.

2. **Aprende la arquitectura:**
   - Lee [docs/03_DISENO_TECNICO_DETALLADO.md](../03_DISENO_TECNICO_DETALLADO.md) (20 min)
   - Entiende cómo fluyen los documentos por Classify → Extract → Normalize

3. **Explora la BD:**
   - Abre [docs/especificaciones/DATA_MODELS_ER_DIAGRAM.md](../especificaciones/DATA_MODELS_ER_DIAGRAM.md)
   - Entérate de tablas de Executions, Classifications, Extractions

4. **Ejecuta tests completos:**
   - Corre `.\tests\api-tests\test-ingest-notasimple.ps1` para un E2E más realista
   - O usa los scripts en `tests/api-tests/` según tu caso de uso

5. **Configura IDE (opcional):**
   - Si usas VS Code, instala extensiones recomendadas:
     - C# (Microsoft)
     - Azure Tools (Microsoft)
     - REST Client (Huachao Mao)
     - Python (Microsoft) — si trabajas con ML

6. **Suscríbete a actualizaciones:**
   - El equipo publica cambios en [docs/auxiliares/](../auxiliares/)
   - Lee `CAMBIOS_UI_CLASSIFICATION.md` y migration guides
...
**¡Listo!** En 15 minutos ya tienes DocumentIA corriendo localmente. 🚀

¿Algo no funcionó? Revisa **Debugging & Tips** o pregunta al equipo.
