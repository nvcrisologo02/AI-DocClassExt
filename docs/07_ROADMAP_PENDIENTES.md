...
| **EP2** | Clasificacion y extraccion AI | DONE | 100% | DI clasificacion + CU extraccion + fallback GPT. Fix preproceso markdown aplicado. Degradacion segura activa. Limiter de concurrencia CU (`SemaphoreSlim`), backoff exponencial con reintentos configurables, telemetría granular (`limiterWaitMs`, `analysis`, `prepare`, `parse`, `attempts`). Feature #99671 completado 2026-05-28. |
...
| C-10 | AB#99768 QA CRUD Validation — TipologiaMapper testeable | **DONE** | (2026-06-08) Agregado parameterless constructor a TipologiaMapper + null-coalescing para ILogger?.LogWarning. Permite instantiación via Moq sin satisfacer ILogger. 9/9 smoke tests CRUD endpoint passing. Ver [docs/auxiliares/AB99768_DOD_CHECKLIST.md](docs/auxiliares/AB99768_DOD_CHECKLIST.md). | AB#99768 |
| C-11 | AB#99078 F6.3 Telemetry — ITelemetryService wrapper + unit tests | **DONE** | (2026-06-08) Creada interfaz ITelemetryService (2 métodos: TrackEvent, TrackMetric) + implementación ApplicationInsightsTelemetryService (wrapper TelemetryClient). Refactorizada PersistirActivity para consumir interfaz en lugar de sealed class. Registrada en Program.cs DI. 11/11 tests unitarios PersistirActivityTests passing (incluyendo 3 nuevos: telemetría, metrics, error handling). Ver [docs/auxiliares/TELEMETRY_UNIT_TESTS_RESOLUTION.md](docs/auxiliares/TELEMETRY_UNIT_TESTS_RESOLUTION.md). | AB#99078 |
| C-12 | AB#99391 GO/NO-GO Production — Plan de validación 7 métricas | **IN PROGRESS** | (2026-06-08 init) Documentado plan operativo en 5 fases (Ground Truth, Batch Classification, Cálculo métricas, Consolidación, Decisión). 7 criterios definidos con umbrales exactos: Precision (60%/65%), Recall (70%), F1 (0.65), Fallback (≤25%), P95 (≤90s), Coste (≤€0.10/doc). Baseline provisional de Mayo 8-10 documentada. Scripts batch y análisis pendientes de implementación. Ver [docs/auxiliares/AB99391_VALIDACION_GO_NO_GO_2026-06-08.md](docs/auxiliares/AB99391_VALIDACION_GO_NO_GO_2026-06-08.md) y [docs/auxiliares/PLAN_EJECUCION_VALIDACION_99391.md](docs/auxiliares/PLAN_EJECUCION_VALIDACION_99391.md). | AB#99391 |
