...
- **`AZURE_ASSET_RESOLVER_WEB_APP_NAME`** (AB#99997) — per-environment AssetResolver app name, used only for the final validation step. Before this fix the value was hardcoded to the **PRODUCTION** AssetResolver (`srbwebpluginassetresolver`) regardless of `targetEnvironment`, so validating dev/pre failed with a cryptic error (the script tried to read app settings from the wrong app). Now each environment resolves its own AssetResolver name (dev: `srbwebpluginassetresolverdev` · pre: `srbwebpluginassetresolverpre` · prod: `srbwebpluginassetresolver`), matching `azure-pipelines.yml`.
