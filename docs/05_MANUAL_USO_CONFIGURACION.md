...
Detalle tecnico del contrato (`Grupos` / `ActivosPorGrupo`) en [ESPECIFICACION_PLUGIN_ASSETRESOLVER.md](especificaciones/ESPECIFICACION_PLUGIN_ASSETRESOLVER.md#54-grupos-de-criterios-multi-activo).
...
Sin configuracion explicita, la aplicacion apunta a `localhost`. En Azure, el pipeline `azure-pipelines-admin.yml` fija `FunctionsAdminApi__BaseUrl` con el valor correcto de cada entorno (dev/pre/prod) al desplegar.
