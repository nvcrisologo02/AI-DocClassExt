...
| Documento en formato Office (XLSX, PPTX, DOCX) o imagen | Se procesa con normalidad. El recorte por paginas se omite (solo aplica a PDF) y el documento completo se envia a Document Intelligence Layout por `BlobPath`, que autodetecta el formato y devuelve markdown. En Office se extrae el texto embebido; **no** se hace OCR de imagenes incrustadas, por lo que una presentacion hecha de capturas de pantalla producira poco o ningun texto. |
| Prompt o resumen solicitado sin texto disponible | Estado final `SIN_CONTENIDO_DOCUMENTO`. No se invoca al modelo. Antes de esta guarda se le llamaba con el contenido vacio y devolvia respuestas del tipo "No has incluido el documento" que se persistian como resumen valido con confianza 1.0. |
...
| **SIN_CONTENIDO_DOCUMENTO** | Estado final de un documento para el que se pidio un prompt o un resumen y no se pudo obtener texto por ninguna via (extraccion, layout previo ni extraccion bajo demanda). El modelo no se invoca y no se persiste resumen: es preferible un fallo explicito a un resumen inventado sobre un documento que el sistema nunca llego a leer. Causas habituales: documento ilegible o corrupto, o Document Intelligence no disponible. |
...
| HU6 | Configuracion de reglas JSON | Como administrador de tipologias, quiero definir reglas de validacion en JSON sin cambiar codigo. | Archivos `{tipologia}.validation.json` con campos, tipos de regla y severidades. | DONE |
...
| HU8 | Gestion de tipologias | Como administrador de tipologias, quiero crear, editar, publicar y retirar tipologias desde un portal. | CRUD + lifecycle Draft→Published→Retired via API y Blazor Admin. | DONE |
